(() => {
  ...
  async function createHbbAccount(event) {
    ...
    const ownerEmail=String(fields.get("owner_email") || "").trim();
    ...
    if (error) return alert("Could not add HBB account: " + error.message + (String(error.message).includes("Two-step") || String(error.message).includes("owner access") ? "\n\nOpen Shizuku Admin, complete two-step verification, then return here." : ""));
    const { data:inviteResult, error:inviteError } = await productionClient.functions.invoke("invite-hbb-owner", {
      body:{ email:ownerEmail, redirect_to:`${window.location.origin}/hbb-onboarding.html` }
    });
  ...
  function render() {
    ...
    const myCount=Number(capacity?.malaysia_hbb_count||0),myLimit=Number(capacity?.malaysia_hbb_limit||2);
