(() => {
  ...
  function jwtUsesSupportedLogin(session){
    try { const encoded=(session?.access_token?.split(".")[1]||"").replace(/-/g,"+").replace(/_/g,"/"); const claims=JSON.parse(atob(encoded.padEnd(Math.ceil(encoded.length/4)*4,"="))); return claims.aal==="aal2"||(Array.isArray(claims.amr)&&claims.amr.some(item=>item?.method==="otp"||item?.method==="password")); } catch(_){ return false; }
  }
  ...
  async function savePassword(){
    ...
    state.password=state.confirm=""; await acceptInvite();
  }
